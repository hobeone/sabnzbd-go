...
| Glitter web UI | Phase 12 complete; templates ported to `html/template` and served at `/`. Default English catalog loaded (Step 12.10). | Some upstream feature flags do not yet have backing `RenderContext` fields — when porters hit one, the gated upstream content is omitted (not faked-as-true). See deferrals just below. |
